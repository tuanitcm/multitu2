...
import { 
  ...
  ArrowRight, ArrowLeft, Box, Github, Info, HelpCircle, Ruler
} from 'lucide-react';
...
const App: React.FC = () => {
  // Initialize state from URL query param if present
  ...
  // Update URL when tool changes (Routing)
  const handleToolSelect = (toolId: string | null) => {
    ...
    if (toolId) {
      ...
      window.history.pushState({ toolId: null }, '', newUrl);
    }
  };
  ...
  // Update Head (Title, Meta, Schema) for SEO
  useEffect(() => {
    if (activeTool) {
      // 1. Update Title & Meta
      const title = `${activeTool.title} - Công cụ Online Miễn phí`;
      const url = `${window.location.origin}${window.location.pathname}?tool=${activeTool.id}`;
      updateMetaTags(title, activeTool.description, url);

      // 2. Construct Graph for Structured Data (SoftwareApp + FAQPage)
      const graphData: any[] = [
        {
      ...
      // 1. Reset Title & Meta for Homepage
      const title = "MultiTools - Bộ công cụ trực tuyến miễn phí";
      ...
      // 2. Inject Collection Structured Data
      injectStructuredData({
        "@context": "https://schema.org",
        "@type": "WebApplication",
        "name": "MultiTools",
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <main className="flex-grow pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        ...
        {activeTool ? (
          // TOOL VIEW - Article semantics
          <article className="animate-in slide-in-from-bottom-8 duration-500 max-w-4xl mx-auto">
             <button 
                onClick={() => handleToolSelect(null)}
                className="flex items-center gap-2 text-slate-400 hover:text-indigo-400 mb-6 transition-colors group"
             >
                   ...
                   <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-xs font-medium text-slate-400 uppercase tracking-wider">
                          {categoriesList.find(c => c.id === activeTool.category)?.label}
                        </span>
                      </div>
